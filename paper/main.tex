...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig1_hero_mechanism.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig2_specialization_emergence.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.7\textwidth]{fig3_causality_heatmap.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig5_crossllm.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig4_practical_benefit.pdf}
...
% -----------------------------------------------------------------------------
\section{v2: Architecture Comparison}
\label{app:architecture}

In v2, we compare our Competitive Specialist Ecosystem (CSE) against alternative population architectures, all using the same inner algorithm (Thompson Sampling). This isolates the effect of \textbf{population structure} on emergent specialization.

\subsection{Baseline Architectures}

\begin{table}[h]
\centering
\caption{Architecture comparison results ($n=3$ seeds, 12 agents, 60 generations).}
\begin{tabular}{lcccc}
\toprule
\textbf{Architecture} & \textbf{Coverage} & \textbf{SCI} & \textbf{Tokens} & \textbf{Efficiency} \\
\midrule
Independent Training & 93\% $\pm$ 9\% & 0.138 & 1,200 & 0.778 \\
MARL Shared Critic & 20\% $\pm$ 0\% & 1.000 & 1,200 & 0.167 \\
Tournament Selection & 87\% $\pm$ 9\% & 0.126 & 4,800 & 0.181 \\
Market-Based Bidding & 87\% $\pm$ 9\% & 0.054 & 1,200 & 0.722 \\
\textbf{CSE (Ours)} & \textbf{93\%} $\pm$ 9\% & \textbf{0.086} & 14,400 & 0.065 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Key Findings}:
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{MARL Shared Critic fails completely} (20\% coverage) --- proves shared learning causes homogenization
    \item \textbf{Competition enables specialization} --- all competitive methods achieve $>$80\% coverage
    \item \textbf{CSE achieves best diversity} (SCI=0.086) through fitness sharing
\end{itemize}

\subsection{Isoperformance Analysis}

Question: How many tokens to reach target coverage?

\begin{table}[h]
\centering
\caption{Tokens required to reach target coverage ($n=5$ seeds).}
\begin{tabular}{lcc}
\toprule
\textbf{Target} & \textbf{Independent} & \textbf{CSE} \\
\midrule
60\% coverage & 176 $\pm$ 79 & 1,248 $\pm$ 384 \\
80\% coverage & 368 $\pm$ 164 & 2,544 $\pm$ 1,354 \\
100\% coverage & 4,368 $\pm$ 4,599 (60\% success) & 14,016 $\pm$ 20,301 (\textbf{100\% success}) \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key Insight}: CSE \textbf{guarantees} 100\% coverage (100\% success rate) while Independent only achieves it 60\% of the time. The reliability of full coverage is CSE's key advantage.

\subsection{Cost Comparison}

\begin{table}[h]
\centering
\caption{Cost comparison for creating $N=8$ specialists.}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{GPU Hours} & \textbf{API Cost} & \textbf{Training Data} \\
\midrule
LoRA Fine-Tuning $\times N$ & 16h & \$0 & 1000+ examples/specialist \\
OPRO $\times N$ & 0h & \$20--40 & 0 (emergent) \\
\textbf{Our CSE (1 run)} & 0h & \textbf{\$5--15} & \textbf{0 (emergent)} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{fig6_cost_benefit.pdf}
