...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Practical benefit}: Specialized populations outperform generalists by +60.8pp $\pm$ 9.6pp ($n=5$) with 5-7 task break-even (Section~\ref{sec:realworld}).
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Synthetic rule domains with cognitive science grounding. Rules are categorized by their reliance on prior knowledge.}
\label{tab:rules}
\small
\begin{tabular}{llll}
\toprule
\textbf{Category} & \textbf{Rule} & \textbf{Description} & \textbf{Cognitive Basis} \\
\midrule
\multirow{3}{*}{Purely Arbitrary}
  & POSITION & Answer at position B & Serial position \\
  & PATTERN & ABAB alternation & Gestalt psychology \\
  & MATH\_MOD & Length mod 3 = 1 & Number cognition \\
\midrule
\multirow{3}{*}{Semi-Arbitrary}
  & VOWEL\_START & Starts with vowel & Phonemic awareness \\
  & RHYME & Rhymes with CAT & Phonological processing \\
  & ALPHABET & Closest to M & Orthographic processing \\
\midrule
\multirow{2}{*}{Knowledge-Aided}
  & ANIMATE & Living thing & Category-specific \\
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Level 0}: No strategy (random guessing)
    \item \textbf{Level 1}: Hint (one-sentence guidance, $\sim$30 chars)
    \item \textbf{Level 2}: Partial (detailed method, $\sim$200 chars)
    \item \textbf{Level 3}: Full (complete strategy with examples, $\sim$500+ chars)
\end{itemize}

\paragraph{Exclusivity Mechanism.} Once an agent reaches Level 3 in any rule, they can only accumulate further strategies in that rule. This enforces specialization and prevents generalist convergence.

\paragraph{Option B+ Initialization.} To solve the cold-start problem, each agent begins with Level 1 in one randomly-assigned rule, providing initial differentiation.
...
We provide a formal mathematical analysis establishing convergence guarantees for the preference specialization mechanism.
...
\paragraph{Prompt Swap Test.} We test whether prompts \textit{cause} performance differences by swapping specialists' prompts and measuring accuracy changes. A test ``passes'' if the specialist-on-own-rule accuracy exceeds specialist-on-other-rule accuracy by $>$30 percentage points.

\begin{table}[h]
\centering
\caption{10-seed unified validation results. All metrics computed with gemini-2.5-flash.}
\label{tab:multiseed}
\begin{tabular}{lcc}
\toprule
...
Cohen's $d$ & \textbf{2.66} & Huge effect size \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Baseline comparison demonstrating strategy importance. CORRECT\_PROMPT achieves perfect accuracy; ablations confirm strategy is necessary.}
\label{tab:baselines}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[h]
\centering
\caption{Cross-LLM validation across three major providers. All exceed 30\% gap threshold.}
\label{tab:crossllm}
\begin{tabular}{llccc}
\toprule
...
% =============================================================================
\section{Practical Benefit: Beyond Synthetic Rules}
\label{sec:realworld}

\subsection{5-Condition Comparison}

We compare specialized populations against single generalist agents:

\begin{table}[h]
\centering
\caption{Practical benefit comparison ($n=5$ runs). Oracle routing achieves +60.8pp $\pm$ 9.6pp improvement.}
\label{tab:practical}
\begin{tabular}{lccc}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{API Calls} & \textbf{$\Delta$} \\
\midrule
SINGLE\_GENERALIST & 29.2\% & 24 & -- \\
\textbf{ORACLE\_ROUTING} & \textbf{90.0\%} & 24 & \textbf{+60.8pp} \\
CONFIDENCE\_ROUTING & 41.7\% & 216 & +12.5pp \\
ENSEMBLE & 45.8\% & 192 & +16.6pp \\
\bottomrule
\end{tabular}
\end{table}

Oracle routing (perfect task-type labels) achieves \textbf{+60.8 $\pm$ 9.6 percentage points improvement} ($n=5$, 95\% CI: [48.9, 72.7]) at no additional cost---demonstrating the value of specialization when routing is accurate.
...
\begin{itemize}[leftmargin=*,topsep=0pt,itemsep=2pt]
    ...
    \item \textbf{Accuracy improvement}: +60.8pp $\pm$ 9.6pp (Oracle routing, $n=5$)
\end{itemize}
...
\begin{enumerate}[leftmargin=*,topsep=0pt,itemsep=2pt]
    \item \textbf{Routing overhead}: Practical benefit requires task-type prediction. Oracle routing (+60.8pp) is an upper bound.
...
\STATE Option B+: $\forall i$: set $s_{i,\text{rand}(R)} \leftarrow 1$
\FOR{$t = 1$ to $G$}
