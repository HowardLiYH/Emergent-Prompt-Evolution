...
\begin{itemize}
    ...
    \item \textbf{Part III} (Mechanism): How competition produces specialists
    \item \textbf{Part IV} (Theory): Formal theorems and proofs
    \item \textbf{Part V} (Experiments): Empirical validation and statistics
    \item \textbf{Part VI} (Applications): Practical deployment and case studies
    \item \textbf{Part VII} (Significance): Why this matters for AI
\end{itemize}
\end{abstract}
...
\begin{tikzpicture}[scale=1.2]
    ...
    % Traditional tradeoff curves
    \draw[blue, thick, domain=0:4.5] plot (\x, {2.8 - 0.12*(\x-2.5)^2});
    \draw[red, thick, domain=0:4.5] plot (\x, {0.5 + 0.5*\x});
    
    % Shaded "tradeoff zone"
    \fill[gray!20] (0.5, 1.5) -- (2.5, 2.8) -- (4, 2.5) -- (4, 1.5) -- cycle;
    \node[gray, font=\scriptsize] at (2.5, 1.8) {Traditional Tradeoff};

    % Labels for traditional curves
    \node[blue, right, font=\small] at (4.6, 2.2) {Generalist};
    \node[red, right, font=\small] at (4.6, 2.75) {Specialist};

    % OUR APPROACH - above the tradeoff!
    \filldraw[green!50!black] (4, 3.8) circle (4pt);
    \draw[green!50!black, thick, dashed] (4, 2.5) -- (4, 3.8);
    \node[green!50!black, right, font=\small\bfseries] at (4.1, 3.8) {Our Approach};
    \node[green!50!black, right, font=\scriptsize] at (4.1, 3.5) {(Specialists + Routing)};
    
    % Arrow showing we escape the tradeoff
    \draw[->, green!50!black, thick] (3.2, 2.6) to[bend left=20] (3.8, 3.6);
\end{tikzpicture}
\end{center}
...
How do we \textit{prove} that our agents have preference (not capability)? Through a falsification experiment:

\begin{table}[H]
\centering
\caption{The falsification experiment: removing the prompt reveals preference vs. capability.}
\begin{tabular}{lcc}
\toprule
\textbf{Condition} & \textbf{Accuracy} & \textbf{Interpretation} \\
\midrule
With L3 Strategy Prompt & 95\% & High performance with guidance \\
Without Strategy Prompt & 30\% & Near-random without guidance \\
\midrule
\textbf{Performance Drop} & \textbf{65pp} & Proves it's \textit{preference} \\
\bottomrule
\end{tabular}
\end{table}

\begin{warning}
If the 65pp drop did NOT occur---if the agent performed equally with or without the prompt---that would prove the specialization was \textbf{capability} (learned into weights, perhaps through exposure during pretraining). Our large drop confirms it's \textbf{preference}.
\end{warning}
...
Accuracy Gain & +64.2pp $\pm$ 2.3pp & Specialists reach theoretical ceiling (100\%) \\
...
\begin{tikzpicture}[node distance=3.5cm]
    ...
    % Arrow
    \draw[->, ultra thick, blue!70!black] (before) -- node[above, font=\bfseries] {+64.2pp} node[below, font=\small] {Oracle routing} (after);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=0.9]
    ...
    % Years
    \foreach \x/\year in {1/2017, 3/2019, 5/2021, 7/2022, 9/2023, 11/2024, 13/2025} {
        \draw (\x, -0.1) -- (\x, 0.1);
...
\begin{enumerate}
    \item \textbf{Paper 1: Emergent Specialization in Trading Agents}---demonstrated belief-based specialization in financial markets
...
Paper 1 (Emergent Specialization in Trading Agents) used \textbf{Thompson Sampling} as the belief-update mechanism. Our competitive framework can be viewed through the same lens.
...
\begin{whyitmatters}
If specialists were just ``lucky'' (happened to be assigned to rules they were already good at), the off-diagonal performance would be similar to diagonal. The 70.7pp gap proves otherwise.
\end{whyitmatters}
...
Practical Benefit & +64.2pp & [61.3pp, 67.0pp] \\
...
\midrule
NO\_PROMPT & 5.0\% & -95.0pp & Random guessing \\
RANDOM\_PROMPT & 15.0\% & -85.0pp & Wrong strategy hurts \\
WRONG\_PROMPT & 20.0\% & -80.0pp & Mismatched specialty \\
...
\begin{keypoint}
The 95pp gap between NO\_PROMPT (5\%) and CORRECT\_PROMPT (100\%) proves that:
\begin{enumerate}
...
\caption{\textbf{Practical benefit comparison.} Oracle routing achieves 100\% accuracy, demonstrating that evolved specialists reach the theoretical ceiling. The +64.2pp improvement over the generalist baseline represents the maximum extractable value from correct routing.}
...
\rowcolor{green!10} \textbf{ORACLE\_ROUTING} & \textbf{100.0\%} & 24 & \textbf{+64.2pp} \\
CONFIDENCE\_ROUTING & 41.7\% & 216 & +5.9pp \\
ENSEMBLE & 29.2\% & 192 & -6.6pp \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    A: Scale up! Use $N \approx 3R$ agents and proportionally more generations. We tested up to $R=16$ with $N=48$.
...
\textbf{Oracle Improvement} & -- & +64.2pp & +5.9pp & -6.6pp \\
\bottomrule
\end{tabular}
\end{table}
