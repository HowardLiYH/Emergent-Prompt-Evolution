...
\begin{tikzpicture}[scale=0.9]
    % Axes
    \draw[->] (0,0) -- (8,0) node[right] {Confidence $c$};
    \draw[->] (0,0) -- (0,4.5) node[above] {Density $p(c)$};

    % X-axis labels
    \foreach \x/\label in {0/0, 2/0.25, 4/0.5, 6/0.75, 8/1.0} {
        \draw (\x, -0.1) -- (\x, 0.1);
        \node[below] at (\x, -0.1) {\small \label};
    }

    % Beta(1,4) - L0: skewed left (mass near 0)
    \draw[thick, red!70!black, domain=0.1:7.9, samples=100]
        plot (\x, {4 * (1 - \x/8)^3});
    \node[red!70!black, right] at (0.5, 3.8) {\textbf{L0: Beta(1,4)}};
    \node[red!70!black, right] at (0.5, 3.3) {\small mean=0.20};

    % Beta(2,3) - L1
    \draw[thick, orange!80!black, domain=0.1:7.9, samples=100]
        plot (\x, {2.5 * (\x/8) * (1 - \x/8)^2});
    \node[orange!80!black] at (2.5, 1.8) {\small \textbf{L1}};

    % Beta(3,2) - L2
    \draw[thick, blue!60!black, domain=0.1:7.9, samples=100]
        plot (\x, {2.5 * (\x/8)^2 * (1 - \x/8)});
    \node[blue!60!black] at (5.5, 1.8) {\small \textbf{L2}};

    % Beta(4,1) - L3: skewed right (mass near 1)
    \draw[thick, green!50!black, domain=0.1:7.9, samples=100]
        plot (\x, {4 * (\x/8)^3});
    \node[green!50!black, left] at (7.5, 3.8) {\textbf{L3: Beta(4,1)}};
    \node[green!50!black, left] at (7.5, 3.3) {\small mean=0.80};

    % Uniform reference (dashed)
    \draw[dashed, gray, thick] (0, 1) -- (8, 1);
    \node[gray, right] at (8.1, 1) {\small Uniform};

    % Annotation
    \draw[<->, thick] (1, 0.5) -- (7, 0.5);
    \node[below] at (4, 0.3) {\small \textit{Higher level $\Rightarrow$ distribution shifts right}};
\end{tikzpicture}
