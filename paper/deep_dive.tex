...
\begin{tikzpicture}[scale=1.2]
    % Axes
    \draw[->, thick] (0,0) -- (5,0) node[right] {Specialization};
    \draw[->, thick] (0,0) -- (0,4) node[above] {Performance};

    % Curves
    \draw[blue, thick, domain=0:4.5] plot (\x, {3.5 - 0.15*(\x-2.5)^2});
    \draw[red, thick, domain=0:4.5] plot (\x, {0.5 + 0.7*\x});

    % Labels - positioned at end of each curve to avoid overlap
    \node[blue, right, font=\small] at (4.6, 2.85) {Broad Task Coverage};
    \node[red, right, font=\small] at (4.6, 3.65) {Single Task Performance};

    % Optimal point
    \filldraw[green!50!black] (2.5, 3.5) circle (3pt);
    \node[green!50!black, above right, font=\small] at (2.6, 3.5) {Sweet Spot?};
\end{tikzpicture}
\end{center}
