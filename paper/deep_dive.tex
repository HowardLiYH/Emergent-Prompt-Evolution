...
\begin{warning}[Understanding ``10 Seeds'' and ``70.7\% Pass Rate'']
\textbf{What are ``10 seeds''?}

10 seeds = 10 \textbf{independent experiments}, each run from scratch with different random initialization:
\begin{center}
\begin{tabular}{ll}
Seed 1 & Run full experiment (100 generations, 12 agents) $\to$ Results \\
Seed 2 & Run full experiment again (different random init) $\to$ Results \\
$\vdots$ & $\vdots$ \\
Seed 10 & Run full experiment $\to$ Results \\
\textbf{Average} & Combine all 10 for statistical confidence \\
\end{tabular}
\end{center}

\textbf{What is ``70.7\% pass rate''?}

Each experiment runs \textbf{56 pairwise comparisons} (7 specialists $\times$ 8 rules):

\begin{center}
\small
\begin{tabular}{llcl}
\toprule
\textbf{Test} & \textbf{Comparison} & \textbf{Scores} & \textbf{Pass?} \\
\midrule
1 & RHYME specialist on RHYME vs POSITION & 86\% vs 16\% & \textcolor{green!60!black}{\checkmark} Pass \\
2 & RHYME specialist on RHYME vs ANIMATE & 86\% vs 25\% & \textcolor{green!60!black}{\checkmark} Pass \\
3 & RHYME specialist on RHYME vs PATTERN & 86\% vs 29\% & \textcolor{green!60!black}{\checkmark} Pass \\
$\vdots$ & $\vdots$ & $\vdots$ & $\vdots$ \\
56 & INVERSE specialist on INVERSE vs ANIMATE & 88\% vs 27\% & \textcolor{green!60!black}{\checkmark} Pass \\
\bottomrule
\end{tabular}
\end{center}

\textbf{``Pass''} = specialist scores HIGHER on their own rule than on the comparison rule.

\textbf{70.7\%} = 70.7\% of the 560 total comparisons (56 per seed $\times$ 10 seeds) showed the expected pattern.

\textbf{Why not 100\%?} LLM stochasticity + some rules are similar (e.g., PATTERN vs POSITION have similar strategies).
\end{warning}
