...
\subsubsection{What Does ``Statistical Power $> 0.99$'' Mean?}

\textbf{Power} = probability of detecting a real effect if it exists.

\begin{center}
\begin{tabular}{cl}
\toprule
\textbf{Power} & \textbf{Meaning} \\
\midrule
0.80 & Standard threshold (80\% chance to detect effect) \\
0.90 & Strong (90\% chance) \\
\textbf{0.99} & \textbf{Excellent} (99\% chance---we almost certainly detect it) \\
\bottomrule
\end{tabular}
\end{center}

With power $> 0.99$, if specialization is real, we have a 99\%+ chance of detecting it. We did detect it, so this is strong evidence.

\subsubsection{What Is Holm-Bonferroni Correction?}

When you run \textbf{many tests}, some will be ``significant'' by chance (false positives).

\textbf{Example}: If you run 100 tests at $\alpha = 0.05$, you expect $\sim$5 false positives.

Holm-Bonferroni \textbf{adjusts the threshold} to control for this:
\begin{itemize}
    \item More tests $\to$ stricter threshold
    \item Our claims survive this correction $\to$ they're not false positives
\end{itemize}
