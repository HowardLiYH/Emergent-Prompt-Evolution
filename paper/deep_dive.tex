...
\texttt{TEMPLATES} is a pre-defined lookup table mapping each (rule, level) pair to a specific prompt string. Given rule $r$ and level $s$:
\[
\text{prompt} = \text{TEMPLATES}[r][s]
\]
This is a simple lookup, not a generation process. The full library is defined in \texttt{src/genesis/rule\_strategies.py}.

\begin{example}[RHYME Rule Templates]
\small
\begin{center}
\begin{tabular}{cp{9cm}}
\toprule
\textbf{Level} & \textbf{Prompt Text} \\
\midrule
L0 & \textit{(empty---no hint)} \\
L1 & ``Listen to how the words sound, not what they mean.'' \\
L2 & ``One option rhymes with CAT. Find the '-at' ending sound.'' \\
L3 & ``You are a RHYME SPECIALIST. The correct answer ALWAYS rhymes with 'CAT'. STEP-BY-STEP: 1. Check each option for '-at' sound...'' \\
\bottomrule
\end{tabular}
\end{center}
\end{example}
...
\textbf{4. Agent state}: Each agent's full state is a vector $\mathbf{s}_i = (s_{i,1}, \ldots, s_{i,R})$, yielding $4^8 = 65{,}536$ possible configurations.
