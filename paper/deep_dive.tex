...
\begin{keypoint}
\textbf{On Originality:} The Strategy Concentration Index (SCI) uses the well-known formula for normalized entropy ($1 - H/H_{\max}$), which has been used in ecology and information theory for decades. Our contribution is:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Specialization Index Lower Bound}: The population achieves a minimum level of specialization that increases with competition intensity
    \item \textbf{Mono-Regime Collapse Prevention}: Fitness sharing prevents all agents from converging to a single niche
\end{enumerate}
...
\subsection{Belief Updates vs. Strategy Levels}
