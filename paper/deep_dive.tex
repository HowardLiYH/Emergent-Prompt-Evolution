...
The normalization $s_r/3$ is used here \textit{purely for conceptual comparison} with Paper 1. In the actual implementation, strategy levels are used directly as discrete integers:
\[
s_r \in \{0, 1, 2, 3\} \quad \text{(raw levels, not normalized)}
\]
Each level maps to a specific prompt template:
\begin{center}
\begin{tabular}{cl}
\toprule
\textbf{Level} & \textbf{Prompt Content} \\
\midrule
$s = 0$ & No hint (generalist) \\
$s = 1$ & Vague hint (``might involve rhyming...'') \\
$s = 2$ & Partial strategy (``look for rhyming patterns'') \\
$s = 3$ & Full strategy (``You are a RHYME specialist. Always pick...'') \\
\bottomrule
\end{tabular}
\end{center}
The code never computes $s/3$---it simply uses the integer level to select the appropriate prompt.
\end{remark}

\begin{keypoint}[The Prompt Inventory: Fixed, Finite, and Shared]
\textbf{How many prompts exist?}

The prompt system is a \textbf{fixed inventory} of pre-defined templates:
...
\textbf{What is TEMPLATES?}

\texttt{TEMPLATES} is a pre-defined lookup table mapping each (rule, level) pair to a specific prompt string. It is defined \textit{before} the experiment runs and never changes during evolution.

\begin{example}[TEMPLATES for RHYME Rule]
\begin{center}
\small
...
\midrule
L0 & \textit{(empty string---no hint given)} \\[0.3cm]
L1 & ``Listen to how the words sound, not what they mean. Sound similarity is key.'' \\[0.3cm]
L2 & ``One option rhymes with the keyword CAT. Find the option with the same ending sound '-at'. Words like bat, hat, mat, rat all rhyme with cat.'' \\[0.3cm]
L3 & ``You are a RHYME DETECTION SPECIALIST. Your expertise is identifying words that rhyme with CAT. CORE RULE: The correct answer ALWAYS rhymes with 'CAT' (ends with '-at' sound). STEP-BY-STEP: 1. Say 'CAT' out loud in your mind. 2. Check if each option ends with '-at' sound. 3. Select the option that rhymes. EXAMPLES: fish, bat, tree $\to$ bat...'' \\
\bottomrule
\end{tabular}
\end{center}
\end{example}

The full template library is defined in \texttt{src/genesis/rule\_strategies.py} with 8 rules $\times$ 4 levels = 32 unique prompt strings.

\textbf{Key Properties:}
\begin{enumerate}
    \item \textbf{Deterministic mapping}: Given (rule $r$, level $s$), the prompt is uniquely determined:
    \[
    \text{prompt} = \text{TEMPLATES}[r][s]
    \]
    This is a simple lookup, not a generation process.

    \item \textbf{Shared prompts}: Yes! Multiple agents can have the \textbf{same prompt} if they're at the same level for the same rule. For example, if Agent 1 and Agent 7 both have $s_{\text{RHYME}} = 2$, they receive identical prompts.

    \item \textbf{Pre-authored, not generated}: Prompts are handcrafted by the experimenter, not generated by the LLM. This ensures controlled experiments:
    \begin{itemize}
        \item L1: $\sim$100 characters (vague hint)
        \item L2: $\sim$250 characters (partial strategy)
        \item L3: $\sim$500+ characters (full instruction with examples)
    \end{itemize}

    \item \textbf{Agent state = vector of levels}: An agent's full prompt state is a vector $\mathbf{s}_i = (s_{i,1}, \ldots, s_{i,R})$. The number of possible agent states is $4^8 = 65{,}536$.
\end{enumerate}

\textbf{Example}: Two agents with identical RHYME expertise:
\begin{center}
\begin{tabular}{lll}
\toprule
Agent & $s_{\text{RHYME}}$ & Prompt received for RHYME task \\
\midrule
Agent 3 & 2 & ``Rhymes with CAT. Find the option with '-at' sound...'' \\
Agent 9 & 2 & ``Rhymes with CAT. Find the option with '-at' sound...'' \textit{(identical!)} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{Why fixed prompts?} This design:
\begin{itemize}
    \item Ensures \textbf{reproducibility}: Same level → same prompt → same expected behavior
    \item Enables \textbf{causality testing}: Swap tests are meaningful because prompts are identical
    \item Matches Paper 1's \textbf{method library}: Fixed set of methods available to all agents
\end{itemize}
\end{keypoint}

\begin{remark}[On the Credibility of Pre-Authored Prompts]
\textbf{Potential concern}: ``The prompts are hand-designed---doesn't this undermine the emergence claim?''

\textbf{Answer}: No, and here's why:

\textbf{1. Rules are grounded in cognitive science:}
\begin{center}
\small
\begin{tabular}{lp{8cm}}
\toprule
\textbf{Rule} & \textbf{Cognitive Science Foundation} \\
\midrule
ANIMATE & Category-specific processing (Warrington \& Shallice 1984; Caramazza \& Shelton 1998) \\
RHYME & Phonological awareness (Goswami 2001; Wagner et al. 1987) \\
VOWEL\_START & Phonemic awareness (Treiman \& Zukowski 1991; Adams 1990) \\
PATTERN & Gestalt pattern recognition (Wertheimer 1923) \\
POSITION & Spatial reasoning (Johnson-Laird 1983) \\
MATH\_MOD & Numerical cognition (Dehaene 1997) \\
\bottomrule
\end{tabular}
\end{center}

\textbf{2. Prompts follow pedagogical principles:}
\begin{itemize}
    \item L1 $\to$ L2 $\to$ L3 mirrors \textbf{scaffolded learning}: hint $\to$ partial $\to$ full
    \item This is standard instructional design, not arbitrary wording
\end{itemize}

\textbf{3. Prompts are a controlled variable, not the finding:}
\begin{itemize}
    \item We don't claim to \textit{discover} prompts
    \item We claim agents \textit{differentially acquire} prompts through competition
    \item The \textbf{mechanism} (specialization via competition) is the contribution
\end{itemize}

\textbf{4. Ablation validates emergence, not prompt quality:}
\begin{itemize}
    \item Competition-only produces SCI = 0.773 (without changing prompts)
    \item This proves specialization is emergent, not engineered via prompt design
\end{itemize}

\textbf{5. Cross-LLM validation:}
\begin{itemize}
    \item Same prompts work across Gemini, GPT-4, and Claude
    \item Prompt effectiveness is not model-specific
\end{itemize}

\textbf{Future work}: Explore automated prompt generation (e.g., PromptBreeder) to remove human authoring entirely. However, for controlled experiments, pre-authored prompts are \textit{preferable} because they isolate the variable of interest (specialization mechanism).
\end{remark}

\begin{warning}[Common Misconception: Prompts Are NOT Randomly Assigned]
Prompts are \textbf{deterministically derived from accumulated levels}, not randomly assigned each round.

\textbf{How it works:}
\begin{enumerate}
    \item Each agent maintains a \textbf{persistent strategy vector} $\mathbf{s}_i = (s_{i,1}, \ldots, s_{i,R})$
...
\textbf{Example: Round 23, RHYME task selected}
...
\midrule
Agent 5 & 3 & ``You are a RHYME specialist. Always pick...'' & Expert \\
Agent 8 & 2 & ``Look for '-at' rhyming patterns...'' & Moderate \\
Agent 2 & 0 & (no strategy hint) & None \\
\bottomrule
\end{tabular}
\end{center}

Agent 5 has the ``headstart'' because they \textbf{won previous RHYME competitions}---the advantage is \textbf{earned through competition}, not randomly assigned.

\textbf{Timeline of expertise accumulation:}
\begin{center}
\begin{tikzpicture}[>=stealth, node distance=2.5cm]
    \node[draw, rectangle, rounded corners, fill=red!10] (r1) {Round 1\\RHYME\\Agent 5 wins\\$s: 0 \to 1$};
    \node[draw, rectangle, rounded corners, fill=orange!10, right of=r1] (r7) {Round 7\\RHYME\\Agent 5 wins\\$s: 1 \to 2$};
    \node[draw, rectangle, rounded corners, fill=yellow!10, right of=r7] (r15) {Round 15\\RHYME\\Agent 5 wins\\$s: 2 \to 3$};
    \node[draw, rectangle, rounded corners, fill=green!10, right of=r15] (r23) {Round 23+\\RHYME\\Agent 5 dominates\\(L3 specialist)};
...
\textbf{Key insight}: This is why specialization emerges---early wins create advantages that compound over time, leading to niche dominance through positive feedback.
\end{warning}

\begin{warning}
\textbf{Terminology Clarification:} Paper 1 uses ``belief'' (expected value of Beta distribution), while Paper 2 uses ``confidence'' (LLM self-reported). These are \textit{different concepts}:
\begin{itemize}
    \item \textbf{Paper 1 Belief}: Computed mathematically as $\alpha/(\alpha+\beta)$---fully transparent
    \item \textbf{Paper 2 Confidence}: Self-reported by LLM---a black box (see Section 19.3)
\end{itemize}
Despite using different mechanisms, both serve the same role: the agent with higher belief/confidence is more likely to win.
\end{warning}
...
\begin{tikzpicture}[scale=0.8]
    % Timeline
    \draw[->, thick] (0,0) -- (12,0) node[right] {Rounds};

    % Without exclusivity
    \node[above, font=\bfseries] at (6, 3) {Without Exclusivity: Super-Agent Emergence};

    \node[draw, rectangle, rounded corners, fill=red!20, minimum width=2cm] at (2, 1.5) {Agent 1: L3 RHYME};
    \node[draw, rectangle, rounded corners, fill=red!20, minimum width=2cm] at (5, 1.5) {+ L2 POSITION};
    \node[draw, rectangle, rounded corners, fill=red!20, minimum width=2cm] at (8, 1.5) {+ L3 POSITION};
    \node[draw, rectangle, rounded corners, fill=red!30, minimum width=2.5cm] at (11, 1.5) {L3 in ALL rules};

    \draw[->, thick] (3, 1.5) -- (3.8, 1.5);
    \draw[->, thick] (6.2, 1.5) -- (6.8, 1.5);
    \draw[->, thick] (9.2, 1.5) -- (9.6, 1.5);
\end{tikzpicture}
\end{center}
...
\caption{\textbf{Causality heatmap.} Diagonal cells (matched agent-task) show high accuracy (green/yellow). Off-diagonal cells (mismatched) show low accuracy (purple/blue). The clear contrast proves that prompts \textit{cause} performance.}
...
\begin{whyitmatters}
If specialists were just ``lucky'' (happened to be assigned to rules they were already good at), the off-diagonal performance would be similar to diagonal. The 70.7\% gap proves otherwise.
\end{whyitmatters}

% ----------------------------------------------------------------------------
\section{Statistical Rigor: Meeting NeurIPS Standards}
...
\begin{table}[H]
\centering
\caption{Training cost breakdown.}
\begin{tabular}{ll}
\toprule
...
Generations & 100 \\
Rules & 8 \\
API calls per generation & $\sim$12 (one per agent) \\
\midrule
\textbf{Total API calls} & $\sim$9,600 \\
\textbf{Cost (Gemini free tier)} & \$0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\textbf{Cost-benefit analysis.} Specialists (green) outperform generalists (coral) after the break-even point ($\sim$7 tasks). The shaded region shows cumulative benefit of specialization.}
\label{fig:cost-benefit}
\end{figure}

\textbf{Break-even calculation:}

If training costs 9,600 API calls and each correctly-routed task saves $X$ (e.g., customer service cost):
\[
\text{Break-even tasks} = \frac{9600}{64.2\% \times X / X} \approx 5\text{--}7 \text{ tasks}
\]

\begin{keypoint}
\textbf{ROI Summary:}
\begin{itemize}
    \item Training is essentially free (within API free tier)
    \item Break-even in 5-7 tasks with good routing
    \item Long-term: +64.2\% accuracy improvement indefinitely
\end{itemize}
\end{keypoint}
