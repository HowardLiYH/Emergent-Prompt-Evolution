...
\begin{enumerate}
    \item \textbf{Method Beliefs}: Each agent maintains Beta distributions $\text{Beta}(\alpha_m, \beta_m)$ for each method $m$, representing belief in that method's effectiveness
...
\textbf{Agent Knowledge} & Beta distribution $\text{Beta}(\alpha, \beta)$ & Strategy level $s \in \{0,1,2,3\}$ \\
\textbf{Selection Mechanism} & Thompson Sampling (sample from posterior) & Confidence-based (highest confidence wins) \\
\textbf{Knowledge Update} & Bayesian update: $\alpha \gets \alpha + R$ & Discrete increment: $s \gets s + 1$ \\
...
\begin{itemize}
    ...
    \item \textbf{Beta beliefs} in Paper 1 are like \textbf{Strategy levels} in Paper 2---accumulated expertise
...
\subsubsection{Proposed Ablation Study}

To validate the claim that exclusivity is not the source of specialization, an ablation study could test:

\begin{table}[H]
\centering
\caption{Proposed ablation conditions to test the role of exclusivity.}
\begin{tabular}{llp{6cm}}
\toprule
\textbf{Condition} & \textbf{Exclusivity} & \textbf{Expected Result} \\
\midrule
Full system & Yes & Strong specialization, guaranteed diversity \\
No exclusivity & No & Some specialization, but risk of super-agent \\
No fitness sharing & Yes & Specialization, but crowded niches \\
Competition only & No & Key test: does specialization still emerge? \\
\bottomrule
\end{tabular}
\end{table}

If the ``competition only'' condition still produces meaningful specialization (even if weaker), this would validate the core claim that \textbf{competition is the source of diversity}.
