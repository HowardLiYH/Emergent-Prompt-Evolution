...
\textbf{Is this ``unfair''?}
\begin{itemize}
    \item Each agent gets exactly ONE L1 seed in ONE random rule
    \item Rules are tested uniformly at random---no rule is systematically favored
    \item Over many rounds, initial seeds become less important than earned wins
    \item Experiments show agents frequently \textit{switch} from their seeded rule to a different specialization (see seed-switching analysis)
\end{itemize}
\end{remark}
