...
Accuracy Gain & +47.2pp $\pm$ 10.5pp & Oracle routing vs generalist ($n=3$) \\
...
\node[draw, rectangle, minimum width=3cm, minimum height=1cm] (before) {Single Generalist\\23.6\% accuracy};
\node[draw, rectangle, minimum width=3cm, minimum height=1cm, right=3cm of before] (after) {Specialized Population\\70.8\% accuracy};
\draw[->, thick] (before) -- node[above] {+47.2pp} (after);
\end{tikzpicture}
\end{center}
...
\begin{table}[h]
\centering
\caption{The 8 synthetic rule domains with cognitive grounding.}
\small
\begin{tabular}{llll}
\toprule
\textbf{Rule} & \textbf{Description} & \textbf{Cognitive Basis} & \textbf{Category} \\
\midrule
POSITION & Answer at position B & Serial position & Purely Arbitrary \\
PATTERN & ABAB alternation & Gestalt psychology & Purely Arbitrary \\
MATH\_MOD & Length mod 3 = 1 & Number cognition & Purely Arbitrary \\
VOWEL\_START & Starts with A,E,I,O,U & Phonemic awareness & Semi-Arbitrary \\
...
ANIMATE & Living thing (animal) & Category-specific & Knowledge-Aided \\
...
\begin{enumerate}
    \item \textbf{Controllability}: We know ground truth
    \item \textbf{No Prior Knowledge}: LLM can't cheat with memorized answers
    \item \textbf{Cognitive Grounding}: Rules inspired by cognitive science
\end{enumerate}
...
\subsection{Option B+ Initialization}
...
\begin{warning}
\textbf{+47.2pp $\pm$ 10.5pp at No Extra API Cost ($n=3$)!}

Oracle routing achieves massive improvement with the same number of API calls as baseline.
\end{warning}
...
Accuracy Improvement & +47.2pp $\pm$ 10.5pp \\
ROI & Excellent \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Massive practical benefit}: +47.2pp $\pm$ 10.5pp at no extra cost
...
\begin{itemize}
    \item +47.2pp $\pm$ 10.5pp accuracy (n=3) at no extra cost
...
Option B+ & Init with L1 in random rule \\
