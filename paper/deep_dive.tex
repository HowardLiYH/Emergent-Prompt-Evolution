...
Accuracy Gain & +60.8pp $\pm$ 9.6pp & Oracle routing vs generalist ($n=5$) \\
...
\node[draw, rectangle, minimum width=3cm, minimum height=1cm] (before) {Single Generalist\\29.2\% accuracy};
\node[draw, rectangle, minimum width=3cm, minimum height=1cm, right=3cm of before] (after) {Specialized Population\\90.0\% accuracy};
\draw[->, thick] (before) -- node[above] {+60.8pp} (after);
\end{tikzpicture}
\end{center}
...
\begin{warning}
\textbf{+60.8pp $\pm$ 9.6pp at No Extra API Cost ($n=5$)!}
...
Accuracy Improvement & +60.8pp $\pm$ 9.6pp \\
ROI & Excellent \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Massive practical benefit}: +60.8pp $\pm$ 9.6pp at no extra cost
...
\begin{itemize}
    \item +60.8pp $\pm$ 9.6pp accuracy ($n=5$) at no extra cost
