...
\begin{table}[h]
\caption{Multi-Seed Swap Test Results (n=7 seeds, VERIFIED)}
\label{tab:multiseed}
\centering
\begin{tabular}{lcccc}
\toprule
...
\midrule
Pass Rate & 61.9\% & 0.166 & [46.6\%, 77.2\%] \\
\bottomrule
\end{tabular}
\footnotesize{Seeds 1-3: Gemini, Seeds 4-7: GPT-4o-mini. All real API.}
\end{table}
...
\begin{table}[h]
\caption{FDR-Corrected Swap Test Results (VERIFIED)}
\label{tab:fdr}
\centering
\begin{tabular}{lcc}
\toprule
...
\midrule
Significant pairs (p $<$ 0.05) & 31/56 (55.4\%) & 28/56 (50.0\%) \\
\bottomrule
\end{tabular}
\footnotesize{Z-test for proportions, Benjamini-Hochberg correction}
\end{table}
...
\begin{tabular}{lcc}
\toprule
N Agents & Swap Pass Rate & Unique Specialists \\
\midrule
8 & 83.3\% & 6/8 \\
12 & 83.3\% & 6/8 \\
24 & 66.7\% & 7/8 \\
...
\bottomrule
\end{tabular}
\footnotesize{$^*$API rate limiting affected some tests. Coverage column removed (was simulated).}
\end{table}
